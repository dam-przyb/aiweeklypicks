...
- **Risk**: Supabase local environment might slightly differ from production (hosted).
  - *Mitigation*: Use Staging environment for final E2E pass.
...
- **Risk**: Testing Astro components purely with Unit tests is complex.
  - *Mitigation*: Rely on E2E (Playwright) for visual/rendering correctness of Astro pages.

## 8. Success Criteria

- **Code Coverage**: Target > 70% coverage for `src/lib` and `src/pages/api`.
- **Pass Rate**: 100% of critical path tests (AUTH-*, ADM-*, PUB-*) must pass before deployment.
