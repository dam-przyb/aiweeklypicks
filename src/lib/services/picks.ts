 ...
 */
export async function listPicks(supabase: SupabaseClient, query: PicksListQuery): Promise<PicksListResponseDTO> {
  ...
  try {
    ...
    let req = supabase.from("picks_history").select(COLUMNS.join(","), { count: "exact" });
    ...
    let req = supabase.from("stock_picks").select(select, { count: "exact" });
