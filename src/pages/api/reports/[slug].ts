...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    if (!result) {
      return new Response(
        JSON.stringify({ code: 'not_found', message: 'report not found' }),
    ...
    if (err?.code === 'bad_request') {
      return new Response(
        JSON.stringify({ code: 'bad_request', message: err.message || 'invalid slug' }),
    ...
    return new Response(
      JSON.stringify({ code: 'server_error', message: 'unexpected error' }),
